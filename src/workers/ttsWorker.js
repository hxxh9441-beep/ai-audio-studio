...
// WebGPU أولاً مع fallback تلقائي لـ WASM.
...
let desiredMode = ENGINE_MODES.AUTO // الوضع المختار (auto | webgpu | wasm)
let pendingEngineReinit = false // تغيير الوضع أثناء عمل — يُطبَّق بعد اكتمال الطابور
...
/**
 * تدشين المحرك حسب وضع التوليد المختار:
 *   webgpu → OnnxWebGPURuntime (EP webgpu+wasm) مع fallback تلقائي لـ WASM
 *   wasm   → OnnxWebRuntime (EP wasm فقط) — WASM يُحمَّل من المسار المحلي (أوفلاين)
 *   auto   → WebGPU إن توفر الجهاز، وإلا WASM
 * ملفات WASM (onnxruntime + phonemize) تُخدم محلياً دائماً عبر basePath صريح.
 */
async function ensureEngine() {
  if (engine) return
  post('status', 'initializing')
  const { PiperWebEngine, OnnxWebGPURuntime, OnnxWebRuntime, PhonemizeWebRuntime } = await loadPiper()
  ...
  if (target === 'webgpu') {
    try {
      engine = new PiperWebEngine({
        onnxRuntime: new OnnxWebGPURuntime(runtimeOpts),
        phonemizeRuntime,
        voiceProvider,
      })
      device = 'webgpu'
      ...
      // WebGPU فشل — ننزل لوضع WASM (لا نكسر المحرك)
    }
  }
...
async function processQueue() {
  ...
  busy = false
  // تطبيق وضع التوليد الجديد الذي طُلب أثناء عمل الطابور
  if (pendingEngineReinit && !stopped) {
    pendingEngineReinit = false
    engine = null
    try {
      await ensureEngine()
    } catch {
  ...
  switch (type) {
    ...
    case 'SET_ENGINE_MODE': {
      ...
      if (busy || queue.length) {
        // يوجد توليد جارٍ — يُطبَّق الوضع الجديد فور اكتماله
        pendingEngineReinit = true
        break
      }
      engine = null
      await ensureEngine()
