...
/**
 * قسم تحويل النص إلى كلام (TTS) — Piper محلي:
 ...
 */
export default function TTSSection({ engineMode = 'auto' }) {
  ...
  // أحدث وضع توليد — للوصول إليه داخل معالج حالة الـ worker
    ...
    try {
      ...
      const queue = new AudioQueue(worker, {
        onWorkerStatus: (status, device) => {
          ...
          if (device === 'wasm' && (status === 'ready' || status === 'idle')) {
            setEngineNotice(
              engineModeRef.current === 'wasm'
                ? tr('engine.modeStandardNotice')
                : tr('tts.engineNotice'),
            )
