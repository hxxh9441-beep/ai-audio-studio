...
// ملاحظة: مجلد worker/ لا يُنسخ — غير مستخدم في مسارنا (تشغيل inline عبر
// OnnxWebGPURuntime/OnnxWebRuntime) ويضم ملفات تتجاوز حد Cloudflare Pages (25MB).
...
export default defineConfig({
  ...
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        name: 'الصوت بين يديك | صوت ↔ نص محلي',
        short_name: 'تحويل صوتي',
        description: 'تحويل الكلام إلى نص، والنص إلى كلام — محلياً 100% على جهازك دون إنترنت.',
        theme_color: '#0b1020',
      ...
      workbox: {
        // precache خفيف: الأساسيات فقط (بدون الـ workers/WASM الكبيرة — تُخزَّن عند الطلب)
        globPatterns: ['**/*.{css,html,svg,png,ico,webmanifest}'],
        globIgnores: ['**/onnx/**', '**/piper/**', '**/ort-stt/**'],
        ...
        runtimeCaching: [
          ...
          },
          {
            ...
            // نماذج Piper تُدار عبر Cache API داخل الـ worker نفسه
            urlPattern: /\/onnx\/.*\.wasm|\/piper\/.*|\/ort-stt\/.*|huggingface\.co/,
            handler: 'NetworkOnly',
          },
        ],
      },
  ...
  build: {
    // chrome110 بدل esnext: يمنع esbuild من لف شيفرة CJS داخل eval()
    // (piper-tts-web يعلّق في الـ worker عند البناء بـ esnext)
    target: 'chrome110',
    sourcemap: false,
  },
})
